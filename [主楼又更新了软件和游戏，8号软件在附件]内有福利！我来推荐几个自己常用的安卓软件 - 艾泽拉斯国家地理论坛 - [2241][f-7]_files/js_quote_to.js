...
var txt = (n.innerText ? n.innerText : n.textContent).replace(/(?:Reply to )?\[ (?:Reply|Topic) \] Post by .+? \(\d+-\d+-\d+ \d+:\d+\):?/i,'')

this.quoteData = "[quote]["+(a.pid?'pid':'tid')+"="+(a.pid?a.pid:a.tid)+"]"+(a.pid?'Reply':'Topic')+"[/"+(a.pid?'pid':'tid')+"] [b]Post by "+$('postauthor'+this.currentSelectId).innerHTML+" ("+commonui.time2date(a.posttime,'Y-m-d H:i')+"):[/b]\n\n"+txt+"[/quote]"
