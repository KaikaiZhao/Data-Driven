...
if(c.indexOf('[album')!=-1){
	c = c.replace(/\[album=?([^\]]{0,50})\](.+?)\[\/album\]/gi,function ($0,$1,$2){
		...
		if ($2.match(/\[(?:img|url)\]/i))
			$2.replace(/\]\s*((?:(?:https?:\/\/)|(?:\.\/))[^\[]+)\s*\[/ig,function ($0,$1){if($1.substr(0,2)=='./')$1=__ATTACH_BASE+'/'+$1.substr(2);img.push($1)})
		else
			$2.replace(/(?:^|[^a-zA-Z0-9\-_\+=\.\$;\/\?:@&=#%])((?:https?:\/\/|\.\/)[a-zA-Z0-9\-_\+=\.\$;\/\?:@&=#%]+)/ig,function($0,$1){if($1.substr(0,2)=='./')$1=__ATTACH_BASE+'/'+$1.substr(2);img.push($1)})
		if (img[0]){
			var a = img[0]
			if (a && a.substr(0,2)=='./')
				a = __ATTACH_BASE+'/'+a.substr(2);
			if (a.substr(0,__ATTACH_BASE.length)==__ATTACH_BASE && a.substr(a.length-10)!='.thumb.jpg')
				a+='.thumb.jpg'
			if (!$1)
				$1='查看相册'
...
if(noimg!=2){
	c = c.replace(/\[img(\d{0,3})\](.+?)\[\/img\]/gi,function($0,$1,$2){
		...
		if (src.substr(0,__ATTACH_BASE.length)==__ATTACH_BASE)
			...
			img_count++
			if ((window.__ATTACH_BASE && window.__ATTACH_BASE == 'http://img.ngacn.cc/attachments' && img_count>5) || img_count>40)
	...
	if(c.indexOf('[flash')!=-1){
		c = c.replace(/\[flash\](.+?)\[\/flash\]/gi,function($0,$1){
			...
			if(u.indexOf('pl.bilibili.tv')!=-1 || u.indexOf('static.loli.my')!=-1)
				w=542,h=412
			self.videonum++
			var x = "<a href='"+u+"' title='暂不显示视频' target='_blank'>"+u+"</a>"
...
c = c.replace(/\[@(.{2,20}?)\]/gi,function($0,$1){ return" <a href='/nuke.php?func=ucp&charset=UTF-8&username="+encodeURIComponent($1)+"' class='b'>"+$0+"</a> " } );//[@]
...
if(type == 'img'){
	if (thumb==1){
		orglink = "[<a target='_blank' href='"+__ATTACH_BASE+"/"+url+"' target='_blank'>查看原图</a>]";
		main = "<a href='"+__ATTACH_BASE+"/"+url+"' target='_blank' title='点击查看原图'><img src='"+__ATTACH_BASE+"/"+url+".thumb.jpg' alt=''/></a>";
		}
	else
		main = "<img src='"+__ATTACH_BASE+"/"+url+"' alt=''/>";
	return "<table class='left quote'><tr><td>"+dscp+"<div class='clear'></div>"+main+"<div class='clear'></div>"+orglink+" "+del+"</td></tr></table>";
	}
else{
	return "<table class='quote'><tr><td>"+dscp+" <a href='"+__ATTACH_BASE+"/"+url+(url_utf8_org_name ? '?filename='+decodeURIComponent(url_utf8_org_name) : '')+"' target='_blank' class='green'>"+(url_utf8_org_name ? decodeURIComponent(url_utf8_org_name) : name)+"</a> ("+size+" K) "+del+"</td></tr></table>";
...
},
{
	...
	1:"<b>一个帖子中所有的随机段落只会显示一个</b><br/><br/>[randomblock]<br/>一段内容……<br/>[/randomblock]<br/><br/>[random]<br/>这两段只会显示其中一个……<br/>[/random]<br/>"
