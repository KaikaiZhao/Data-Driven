...
else if (x=navigator.userAgent.indexOf('firefox')!=-1){
	x=u.substr(x+7,1)
	__UA[0]=3
	}
...
this.adminwindow._.addContent(null)
this.adminwindow._.addContent(
	document.createTextNode('某些功能在设置正确的密码后方可使用(有效期一天) '),
	...
	_$('<button/>')._.attr({innerHTML:'确定',type:'button'})._.on('click',function(){
			...
			loader.script(window.__SCRIPTS.md5,function(){cookieFuncs.setCookieInSecond('admin_code', md5(p+'_'+__CURRENT_UID+'_'+Math.floor(__NOW/3600*48)) ,3600*24*7);alert('done')});
			}
		)
	)
tTip.showdscp(e,this.adminwindow);
 ...
 */
commonui.postDisp = function (cC,cSig,cPoster,pos,rmd,cLen,
	lesser,rvrc,tid,pid,aid,
	avatar,honor,regdate,lastlogin,ip,level,mute,medal,postNum,fid,money,site,ifRemark,repu,client,cS,type
	){
cC = $(cC)
if (!cC)
	return;
...
if(type){
	...
	if ((type & 1)!==0)//comment
		return ubbcode.bbscode(cC,0,lesser,rvrc,0,tid,pid,aid)
	}
...
var lite=window.__SMALL_SCREEN?true:false,posterinfo = commonui.dispUserInfo(lite,pos,avatar,honor,regdate,lastlogin,lesser,ip,level,mute,medal,postNum,rvrc,repu,aid,fid,money,site,ifRemark,$('postauthor'+pos).innerHTML)
...
commonui.usernamelink($('postauthor'+pos),aid)//修改链接

if(!window.__LITE.notLoadPAndS && $(cSig) && cLen>30)
	commonui.loadPostSign(cSig,0,lesser,rvrc,1,tid,pid,aid,lite)
else{
	if($(cSig))$(cSig).parentNode.style.display='none'
	cC.innerHTML+='<br/><br/>'
	}
 ...
 */
commonui.dispUserInfo=function(lite,l,avatar,honor,regdate,lastlogin,lpic,userip,level,mute_time,medal,postnum,aurvrc,r,authorid,fid,money,site,remark,authorname){
var x = '',tmp = '',tmp2 = '',d={}
...
d.authorname = "<a href='nuke.php?func=ucp&uid="+authorid+"' id='postauthor"+l+"' class='author b'>"+authorname+"</a><i class='numeric'>("+authorid+")</i>"
...
commonui.loadPostSign = function(cSig,noimg,lesser,rvrc,is_sig,tid,pid,aid,lite)
{
if (window.__LITE.notLoadPAndS)return;
...
if (lite){
	if (typeof(cSig)=='string')cSig = _$(cSig)
	var x = _$('<span/>')._.css('display','none')
	x.innerHTML = cSig.innerHTML
	var y = _$('<button type="button">点击显示签名</button>')._.on('click',function(){
		this.parentNode.visibility='hidden'
		this.parentNode.innerHTML=this.nextSibling.innerHTML
		ubbcode.bbsCode.call(ubbcode,arg);
		})
	cSig._.attr('innerHTML','')._.aC(y,x)._.css('display','')
	return
	}
...
5:{n1:'举报',n2:'举报此贴至版主',on:function(e,a){commonui.logPost(e,a.tid,a.pid)},
	ck:function(a){if ( __GP['rvrc']>20 || __GP['admincheck'])return 1} },
...
19:{u:"/message.php?action=write&touid={authorid}",n1:'短信',n2:'向作者发送短消息',
	ck:function(a){if (__CURRENT_UID)return 1} },
 ...
 * @param 用户id
 */
commonui.usernamelink = function(o,uid){
if(!o)return
